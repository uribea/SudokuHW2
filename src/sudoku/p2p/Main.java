...
public class Main extends SudokuDialog implements NetworkAdapter.MessageListener {
	...
	public void setValues(boolean boo) {
		...
		if (network != null) {
			network.writeFill(values[0], values[1], values[2]);
		}
	}
	...
	public void messageReceived(sudoku.p2p.NetworkAdapter.MessageType type, int x, int y, int z, int[] others) {
		...
		case JOIN:
			...
			if (JOptionPane.showConfirmDialog(msgBar, "Would you like to share your game", "Share",
		...
		case NEW:
			...
			for (int i = 0; i < others.length; i++)
				infoArea.append(others[i] + " ");
			infoArea.append(" \n");
			if (JOptionPane.showConfirmDialog(msgBar, "Would you like to start a New Game with Player", "New Game",
					JOptionPane.YES_NO_OPTION) == 0) {
				newClicked(x, others);
